...
1. **任何技术决策，由 Claude 列出多个方案**，解释每个方案的优缺点与面试价值，让用户做选择。
2. **代码注释和文档用中文**，关键设计点要在注释中写明"为什么这样设计"（面试可讲点）。
3. **每完成一个功能/阶段，主动向用户解释**：做了什么、核心逻辑是什么、面试官可能会怎么问——帮助用户真正吃透代码，而不是只"做出来"。
4. **遇到技术问题时，主动排查并提出解决方案**，修复后说明问题根因（踩坑过程是面试素材）。
